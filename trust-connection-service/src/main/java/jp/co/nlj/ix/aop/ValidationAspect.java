...
import jp.co.nlj.ix.constant.MessageConstant.System;
import jp.co.nlj.ix.constant.MessageConstant.Validate;
import jp.co.nlj.ix.constant.ParamConstant;
import jp.co.nlj.ix.constant.ParamConstant.CharacterSign;
import jp.co.nlj.ix.constant.ParamConstant.CorrectionCode;
...
/**
 * <PRE>
 * ValidationAspectクラスは、バリデーションを行うためのアスペクトクラスです。<BR>
 * </PRE>
 *
...
public class ValidationAspect {
     ...
     * @throws IllegalAccessException アクセスできないフィールドがある場合
     */
    @Before("execution(* jp.co.nlj.ix..*(..))")
    ...
    /**
     * オブジェクトのフィールドを再帰的にバリデーションする。
     *
     * @param obj            バリデーション対象のオブジェクト
     * @param visitedObjects 訪問済みのオブジェクトのセット
     * @throws IllegalAccessException アクセスできないフィールドがある場合
     */
    private void validateFields(Object obj, Set<Object> visitedObjects) throws IllegalAccessException {
        if (obj == null || visitedObjects.contains(obj) || !isPojo(obj.getClass())) {
            return;
        }
        ...
        if (obj instanceof Collection<?>) {
            for (Object item : (Collection<?>) obj) {
                if (item != null && isPojo(item.getClass())) {
    ...
    private void validateEndDate(String fieldName, ValidateField annotation, Object obj, DateTimeFormatter dtf,
        ...
        if (!annotation.endDateField().isEmpty()) {
            ...
            try {
                ...
                validateInputIsArrayOrBoolean(getJsonPropertyValue(endDateField), endDateStr,
                    endDateField.getAnnotation(ValidateField.class));
     ...
     * @return 開始時刻が終了時刻よりも後か等しい場合はtrue
     */
    ...
    private void validateMinField(String fieldName, Object value, ValidateField annotation, Object obj) {
        if (!BaseUtil.isNull(annotation.minField()) && !BaseUtil.isNull(String.valueOf(value))) {
            try {
                ...
                if (value instanceof BigDecimal bigDecimal && bigDecimal.compareTo(minFieldValue) < 0) {
                    throw new NextWebException(
                        new NextAPIError(
